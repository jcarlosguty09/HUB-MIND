...
// Bloque de score personal reutilizable (para atleta Y admin/coach en su propia clase)
function buildMyScoreBlock(dateKey, classId, existingScore = {}) {
  const userId = Auth.getUser()?.id;
  let selectedCategory = existingScore.category || 'rx';

  const block = document.createElement('div');
  block.className = 'score-block my-score-block';
  block.innerHTML = `
    <div class="my-score-title"><i class="ti ti-user-star"></i> Mi score</div>
  ...
  const catBtns      = block.querySelectorAll('.score-cat-btn');
  const scoreInput   = block.querySelector('.score-input');
  const scoreBtn     = block.querySelector('.score-save-btn');
  const completedBtn = block.querySelector('.score-completed-btn');
  const scoreSaved   = block.querySelector('.score-saved');
  ...
  scoreBtn.addEventListener('click', async () => {
    ...
    const ok = await ScoreAPI.save(dateKey, classId, userId, val, existingScore.scoreType || 'high', selectedCategory, false);
    if (ok) flashSaved();
    else showToast('Error al guardar score');
  });

  completedBtn.addEventListener('click', async () => {
    const ok = await ScoreAPI.save(dateKey, classId, userId, '', existingScore.scoreType || 'high', selectedCategory, true);
...
async function renderReports() {
  const contentEl = el('reports-content');

  // Rango por defecto: últimos 30 días
  if (!el('reports-start').value) {
    const end = new Date();
    const start = new Date(); start.setDate(start.getDate() - 29);
    el('reports-start').value = cdmxDateStr(start);
...
async function load() {
    ...
    contentEl.innerHTML = `
      <div class="reports-stats-grid">
        ...
        <div class="admin-stat-card yellow"><i class="ti ti-chart-line"></i><div class="admin-stat-num">${avgPerDay}</div><div class="admin-stat-label">Promedio/día</div></div>
...
function fmtBirthday(dateStr) {
  ...
  // Próximo cumpleaños
  let prox = new Date(hoy.getFullYear(), b.getMonth(), b.getDate(), 12);
  if (prox < hoy) {
    prox = new Date(hoy.getFullYear() + 1, b.getMonth(), b.getDate(), 12);
  }

  // Edad que cumplirá en esa fecha
  const edadProx = prox.getFullYear() - b.getFullYear();
  const dias  = Math.round((prox - hoy) / 86400000);
  const fecha = b.toLocaleDateString('es-MX', { day: 'numeric', month: 'long' });

  if (dias === 0)  return `¡Hoy cumple ${edadProx} años! 🎉`;
  if (dias === 1)  return `Mañana cumple ${edadProx} años · ${fecha}`;
  if (dias <= 30)  return `Cumple en ${dias} días (${edadProx}) · ${fecha}`;
  return `${fecha} · ${edadProx - 1} años`;
}
